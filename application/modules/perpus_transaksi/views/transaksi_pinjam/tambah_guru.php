								...
								<div class="box-body">
										<form action="<?php echo base_url('perpus_transaksi/prosespinjam');?>" method="POST" enctype="multipart/form-data">											
											...
											<div class="pull-right">
												<input type="hidden" name="tambah_guru" value="tambah_guru">
...
<script>
 ...
 <div class="modal fade" id="TableAnggota">
	...
	<table id="myTable-2" class="table table-bordered table-striped">
		...
		<?php $no=1;foreach($guru as $isi){
			...
			<tr>
				...
				<td><?= $isi['nama_guru'];?></td>				
				<td style="width:20%;">
					<button class="btn btn-primary" id="Select_File_2" data_id="<?= $isi['nip'];?>">
					<i class="fa fa-check"> </i> 
					</button>
				</td>
			</tr>
	...
	<!-- /.modal -->
	<script>
	$(".fileSelection2 #Select_File_2").click(function (e) {
		document.getElementsByName('anggota_id')[0].value = $(this).attr("data_id");
		$('#TableAnggota').modal('hide');
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('perpus_transaksi/result_guru');?>",
