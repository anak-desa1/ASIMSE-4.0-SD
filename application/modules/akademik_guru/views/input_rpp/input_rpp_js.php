...
<script type="text/javascript">
    ...
    $('.btn-action').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>tambah_rpp/" + id,
            data: "jenis=tambah",
    ...
    $('.btn-edit').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>edit_rpp/" + id,
            data: "jenis=ubah",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-edit').modal({
    ...
    $('.btn-cetak').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>cetak_rpp/" + id,
            data: "jenis=cetak",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-cetak').modal({
    ...
    $('.btn-delete').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>del_rpp/" + id,
            data: "jenis=hapus",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-delete').modal({
