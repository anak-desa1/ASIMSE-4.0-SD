...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $("#FormKM").submit(function(e) {
            ...
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'tabel_atp' ?>/"  + id + '/' + target;
            $("#LaporanATP").load(url);
        })
    });
    ...
    $('.btn-cetak').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>cetak_atp/"  + id + '/' + target,
            data: "jenis=cetak",
