...
<script type="text/javascript">
    ...
    $('.btn-action').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) ?>/tambah_galery",
            data: "jenis=tambah",
    ...
    $('.btn-edit').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) ?>/edit_galery/" + id,
            data: "jenis=edit",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-edit').modal({
    ...
    $('.btn-detail').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) ?>/detail_galery/" + id,
            data: "jenis=detail",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-detail').modal({
    ...
    $('.btn-plus').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) ?>/tambah_galery_plus/" + id,
            data: "jenis=plus",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-plus').modal({
    ...
    $('.btn-editplus').on('click', function() {
        show_loading();
        id = $(this).data('id');
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) ?>/edit_galery_plus/" + id,
            data: "jenis=edit",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-editplus').modal({
                    backdrop: 'static',
                    keyboard: false
                }, 'show');
                // validate_form();
