...
<script>
    $(document).ready(function() {
        $("#FormSumatif").submit(function(e) {
            e.preventDefault();
            var id = $("#data").val();
            // console.log(id);
            var url = "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') . 'data_sumatif' ?>/" + id;
            $("#sumatif").load(url);
        })
    });
</script>
...
<script type="text/javascript">
    ...
    $('.btn-action-2').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>edit_atp/" + id,
            data: "jenis=ubah",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-action').modal({
    ...
    $('.btn-action-3').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>edit_2/" + id,
            data: "jenis=tambah",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-action').modal({
