...
<script type="text/javascript">
    ...
    $('.btn-action').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/tambah_tugas/<?= $this->uri->segment(4, 0) ?>",
            data: "jenis=tambah",
    ...
    $('.btn-edit').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/edit_tugas/" + id,
            data: "jenis=edit",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-edit').modal({
    ...
    $('.btn-delete').on('click', function() {
        ...
        xhr = $.ajax({
            method: "POST",
            url: "<?= base_url() . $this->uri->segment(1, 0) . $this->uri->slash_segment(2, 'both') ?>/delete_tugas/" + id,
            data: "jenis=delete",
            success: function(response) {
                $('.tampil-modal').html(response);
                $('.modal-delete').modal({
